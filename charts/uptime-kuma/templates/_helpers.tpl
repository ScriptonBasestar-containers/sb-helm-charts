...
*/}}
{{- define "uptime-kuma.validateMariaDB" -}}
{{- if eq .Values.uptimeKuma.database.type "mariadb" }}
  {{- if not .Values.uptimeKuma.database.mariadb.host }}
    {{- fail "MariaDB host is required when database.type is 'mariadb'. Set uptimeKuma.database.mariadb.host." }}
  {{- end }}
  {{- if not .Values.uptimeKuma.database.mariadb.password }}
    {{- fail "MariaDB password is required when database.type is 'mariadb'. Set uptimeKuma.database.mariadb.password." }}
  {{- end }}
{{- end }}
{{- end }}
