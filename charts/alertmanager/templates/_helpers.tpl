...
{{/*
Create the name of the headless service for clustering
*/}}
{{- define "alertmanager.headlessServiceName" -}}
{{- printf "%s-headless" (include "alertmanager.fullname" .) }}
{{- end }}

{{/*
Generate cluster peers for HA mode
*/}}
{{- define "alertmanager.clusterPeers" -}}
{{- $replicas := int .Values.replicaCount }}
{{- $fullname := include "alertmanager.fullname" . }}
{{- $namespace := .Release.Namespace }}
{{- $headless := include "alertmanager.headlessServiceName" . }}
{{- $peers := list }}
{{- range $i := until $replicas }}
{{- $peers = append $peers (printf "%s-%d.%s.%s.svc.cluster.local:9094" $fullname $i $headless $namespace) }}
{{- end }}
{{- join "," $peers }}
{{- end }}
