...
helm.sh/chart: {{ include "thanos-store.chart" . }}
{{ include "thanos-store.selectorLabels" . }}
{{- if .Chart.AppVersion }}
...
*/}}
{{- define "thanos-store.serviceAccountName" -}}
{{- if .Values.serviceAccount.create }}
{{- default (include "thanos-store.fullname" .) .Values.serviceAccount.name }}
{{- else }}
...
*/}}
{{- define "thanos-store.roleName" -}}
{{- include "thanos-store.fullname" . -}}
{{- end }}

{{/*
Return the name of the RoleBinding
*/}}
{{- define "thanos-store.roleBindingName" -}}
{{- include "thanos-store.fullname" . -}}
{{- end }}
...
*/}}
{{- define "thanos-store.objstoreSecretName" -}}
{{- if .Values.objstore.existingSecret }}
{{- .Values.objstore.existingSecret }}
{{- else }}
{{- include "thanos-store.fullname" . }}-objstore
{{- end }}
{{- end }}
...
*/}}
{{- define "thanos-store.objstoreConfig" -}}
...
config:
  ...
  signature_version2: {{ .Values.objstore.s3.signatureV2 }}
  {{- if .Values.objstore.s3.httpConfig }}
  http_config:
    idle_conn_timeout: {{ .Values.objstore.s3.httpConfig.idleConnTimeout | quote }}
    response_header_timeout: {{ .Values.objstore.s3.httpConfig.responseHeaderTimeout | quote }}
  {{- end }}
